...
// --- MAIN COMPONENT ---
const CoDepartmentDashboard = () => {
  const [activeTab, setActiveTab] = useState('actions');
  const navigate = useNavigate();

  // HARDCODED FOR COMPUTER ENGINEERING
  const currentDept = { name: 'Computer Engineering', code: 'CO' };
  ...
  // Fetch Master List & Filter Strictly for CO
  useEffect(() => {
  ...
  // Fetch Logs & Filter Strictly for CO
  useEffect(() => {
    let isMounted = true;
    const fetchData = async () => {
      try {
        ...
        // STRICT FILTER: Only CO / COMP or Unknowns
  ...
  const summaryData = useMemo(() => {
    ...
    if (realtimeCounts.hasData && realtimeCounts.raw) {
        const raw = realtimeCounts.raw; const dc = 'co'; 
  ...
  const handleDownloadPDF = () => {
    ...
    if (attendanceDateMode === 'today' || attendanceDateMode === 'yesterday') {
        ...
        doc.autoTable({ startY: 60, head: [['#', 'Enrollment', 'Name', 'Mobile', 'Status']], body: attendanceData.present.map((s, i) => [i + 1, s.enrollment, s.name, s.mobile || 'N/A', 'Present']), theme: 'grid', headStyles: { fillColor: [16, 185, 129] }, styles: { fontSize: 10 } });
        ...
        doc.autoTable({ startY: finalY + 5, head: [['#', 'Enrollment', 'Name', 'Mobile', 'Status']], body: attendanceData.absent.map((s, i) => [i + 1, s.enrollment, s.name, s.mobile || 'N/A', 'Absent']), theme: 'grid', headStyles: { fillColor: [244, 63, 94] }, styles: { fontSize: 10 } });
    } else {
        let currentY = 56;
        const yearMaster = masterList.filter(s => s.year === selectedYear);
...
export default CoDepartmentDashboard;